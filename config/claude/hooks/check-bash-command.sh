...
if echo "$COMMAND" | grep -qE 'rm -rf|rm -r|git push --force|git push -f|git reset --hard|git clean -f|git checkout \.|git restore \.|git stash clear|git branch -D|git push.*--delete|git push.*:|git filter-branch|git reflog expire|chmod -R 777|dd |mkfs'; then
  jq -n '{
    decision: "block",
